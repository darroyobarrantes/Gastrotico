...
def cargar_datos(driver):
    try:
        time.sleep(5)
        driver.find_element(By.ID,"geobroaden_opt_in").click()
        driver.find_element(By.ID,"geobroaden_opt_in").click()
        driver.find_element(By.ID,"geobroaden_opt_in").click()
    except:
        print("NA")    
...
#links de cada uno de los resultados para almacenarlos en una lista
##################################################################
def obtener_links(links, rango_i, rango_f, driver, nombres):
    for i in range(rango_i, rango_f):
        ...
        nombre =[elem.get_attribute('text') for elem in restaurant]
        if len(link)!=0:
            links+=link
            nombres+=nombre
...
def abanzar_pagina(rango_i, rango_f, driver, links, nombres):
    siguiente=driver.find_elements(By.XPATH,"//*[@class='nav next disabled']")
    while True:
        time.sleep(5)
        obtener_links(links,rango_i, rango_f, driver, nombres)
        time.sleep(1)
        if len(siguiente)!=0:
            break
        else:
